...
There are several methods to accomodate this. If using FFmpeg to facilitate transcoding, then one method is to convert the PBCore into an ffmetadata document which could be referenced in the transcoding. The attached bash script called <a href="/assets/downloads/pbcore2ffmetadata.sh" download>pbcore2ffmetadata</a> extracts select PBCore data in order to create an ffmetadata file. It can be used in the following way.
